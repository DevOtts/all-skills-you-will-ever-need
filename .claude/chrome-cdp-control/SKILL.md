...
> **About to:** Post the following tweet to @<your-handle> on X:
> > "Testing Brain's new SEMrush connector — domain resolution working end to end. 🧠"
>
> **Confirm to proceed?**
...
- Modal / cookie banner blocking the target → handle the modal first (with the user's approval if it's destructive)
- Element re-rendered with new attributes → re-snapshot the DOM
- Wrong tab → list tabs, verify

**Never** loop "click → fail → click → fail" more than twice. That's the path to accidentally posting garbage to the user's accounts.
...
**Caveat:** OS-level focus must be on Chrome. If the user is in another app, the paste lands in the wrong window. When in doubt, ask them to confirm Chrome is the foreground window.
